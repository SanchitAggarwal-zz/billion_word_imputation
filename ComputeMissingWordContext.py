...
for line in validationset:
	...
	#for i in range(2,len(token)-1):
		...
		if score[0] < score[1]:
			...
			for j in range(0,len(CW)):
				...
				if i >= 3:
					Total_Cosine = compute_score(token[i],CW[j],0) * compute_score(token[i-1],CW[j],1) * compute_score(token[i-2],CW[j],2)
				elif i == 2:
					Total_Cosine = compute_score(token[i],CW[j],0) * compute_score(token[i-1],CW[j],1)
				else:
					Total_Cosine = compute_score(token[i],CW[j],0)

				# Context after the inserted word
				if (i+1) <= len(token)-3:
					Total_Cosine = Total_Cosine * compute_score(CW[j],token[i+1],0) *  compute_score(CW[j],token[i+2],1) * compute_score(CW[j],token[i+3],2)
				elif (i+1) == len(token)-2:
					Total_Cosine = Total_Cosine * compute_score(CW[j],token[i+1],0) * compute_score(CW[j],token[i+2],1)
				else:
					Total_Cosine = Total_Cosine * compute_score(CW[j],token[i+1],0)
	...
	#validationScores.write("*****: "+best_key + ": " + missing_word + "\n")
	validationScores.write("**************************************************************************\n\n")
	...
	if not_inserted:
		try:
			...
			original.insert(1,missing_word)
			pass
	#print original
	print line_id,missing_word
	for word in original:
		word = "".join([ch if ord(ch)<= 128 else "$" for ch in word])
		#word = word.decode('utf-8')
